...
class Bot:
    def __init__(self, mediator):
        ...
        log(f"Starting LearningBot {dt.datetime.now()}")
        self.game = hlt.Game("LearningBot")
        log("Started LearningBot")
    ...
    def play(self):
        while True:
            ...
            log(f'sending state')
            self.mediator.send_state((features, map))

            log(f'receiving action')
            action = self.mediator.receive_action()
...
def main():
    try:
        mediator = Mediator(base_url='http://localhost:5000')
        Bot(mediator).play()
    except:
        import traceback
